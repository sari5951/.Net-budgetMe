...
{
    public class IsManagersAttribute : ActionFilterAttribute
    {
...

        {
            ...
            if (userId != null && userId != "" && int.TryParse(userId, out int thesId1) && thesId1 > 0)
            {
                ...
                if (user != null && (user.UserTypeId == (int)UserTypeDTO.SystemManager||user.UserTypeId == (int)UserTypeDTO.LendersManager))
                {
                    return;
                }
                else
